...
class SequentialDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, dl_format=InputType.POINTWISE, shuffle=False, phase='train'):
        ...
        self.phase = phase
        if config['SSL_AUG'] == 'FEARec' and self.phase == 'train':
    ...
    def data_preprocess(self):
        ...
        if self.config['model'] == 'FEARec':
            # 数据集之间不一样
            if self.config['dataset'] == 'yelp':
                self.static_item_id_list = self.pre_processed_data['business_id_list'].detach().clone()
            else:
    ...
    def _shuffle(self):
        if self.real_time:
            ...
            if self.config['SSL_AUG'] == 'FEARec' and self.phase == 'train':
    ...
    def _get_processed_data(self, index):
        ...
        elif self.config['SSL_AUG'] == 'FEARec' and self.phase == 'train':
            self.fearec_aug(cur_data, index)
        
        return cur_data
    
    def fearec_aug(self, cur_data, index):
    ...
    def cl4srec_aug(self, cur_data):
        ...
        seqs = cur_data['business_id_list']
